...
public class Pepball_gameplay extends JPanel implements KeyListener, ActionListener {
    ...
    public Pepball_gameplay() {
        ...
        timer.start();
 try {
            FileInputStream sound = new FileInputStream(new File("b-breaker\\nier.wav"));
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (play) {
            if (new Rectangle(ballX, ballY, 20, 20).intersects(new Rectangle(PlayerX, 550, 100, 8))) {
                ballYdir = -ballYdir;
                try {
                    FileInputStream sound = new FileInputStream(new File("b-breaker\\8bitgame10.wav"));
                    AudioStream IMusic = new AudioStream(sound);
                    AudioPlayer.player.start(IMusic);
                } catch (IOException x) {
